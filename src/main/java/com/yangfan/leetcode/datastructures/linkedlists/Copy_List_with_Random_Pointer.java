...
public class Copy_List_with_Random_Pointer {
    ...
    public static RandomNode copyRandomListPro(RandomNode head) {
        ...
        RandomNode cur = head;
        RandomNode next = null;  // 缝毛衣的针
        // copy node and link to every node
        // 1 -> 2
        // 1 -> 1'-> 2
        while(cur != null) {
            next = cur.next;  // 先把原链表的next保存下
            cur.next = new RandomNode(cur.val); // 追加复制节点
            cur.next.next = next; // 把原链表追加到复制节点后
            cur = next; // 继续编织下一个节点
        }
        ...
        // set copy node rand
        while(cur != null) {
            next = cur.next.next; // 保存原节点的下一个原节点
            curCopy = cur.next; // 复制节点
            curCopy.random = cur.random != null ? cur.random.next : null; // 复制节点的random 即为原节点random节点 的复制节点
            cur = next;  // 配合循环第一行，一次设置一对节点  n -> n'
        }
        ...
        // split 编织之前保存下一个原节点
        while(cur != null) {
            next = cur.next.next; // 记录 1 -> 1' -> 2 的2节点
            curCopy = cur.next;
            cur.next = next; // 断开了copy节点
            curCopy.next = next != null ? next.next : null; // 设置copy节点的next
            cur = next; // 原链表下一个节点
        }
        return res;
    }
